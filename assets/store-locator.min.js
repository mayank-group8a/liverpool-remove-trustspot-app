...
storeLocator.GMEDataFeed.prototype.getStores = function(a, b, c) {
  console.log("call GMEDataFeed click");
    var d = this,
        e = a.getCenter();
...
storeLocator.Panel.prototype.init_ = function() {
    ...
    this.settings_.locationSearch && (this.locationSearch_ = $('\x3cdiv class\x3d"location-search"\x3e\x3clabel  for\x3d"locationInput" \x3e' + this.settings_.locationSearchLabel + "\x3c/label\x3e\x3cinput id\x3d'locationInput'\x3e\x3c/div\x3e"), this.filter_.append(this.locationSearch_), "undefined" != typeof google.maps.places ? this.initAutocomplete_() :
...
storeLocator.Panel.prototype.toggleFeatureFilter_ = function(a) {
  console.log("call toggleFeatureFilter_ click");
...
storeLocator.Panel.prototype.searchPosition = function(a) {
  console.log("test searchPosition");
    var b = this;
    a = {
        address: a,
...
storeLocator.Panel.prototype.setView = function(a) {
  console.log("test setView");
    this.set("view", a)
};
storeLocator.Panel.prototype.view_changed = function() {
  console.log("test view_changed");
...
storeLocator.Panel.prototype.stores_changed = function() {
   console.log("test stores_changed");
    if (this.get("stores")) {
        var a = this.get("view"),
		...
		var count=0;
      	//console.log('kkk'+c.length);
        //console.log('xxx'+b);
        //console.log('yyy'+!b.contains(c[0].getLocation()));
        ...
        for (var b = function() {
            ...
            if(xxx.contains(c[e].getLocation())){
                var g = c[e].getInfoPanelItem();
                g.store = c[e];
                d && c[e].getId() == d.getId() && $(g).addClass("highlighted");
                
                d && c[e].getId() == d.getId() && $(g).attr("aria-current","true");
        ...
        if( count >= 99 ) {
          this.storeList_.append('\x3cspan class\x3d"narrow-down"\x3eThese are the 100 nearest stores. Move around the map or narrow down location for more results.\x3c/span\x3e');
        }

    }
};
storeLocator.Panel.prototype.selectedStore_changed = function() {
   console.log("test highlighted");
    
    $(".highlighted", this.storeList_).removeAttr("aria-current");
    ...
    if (b) {
        ...
        this.storeList_.find("#store-" + b.getId()).attr("aria-current","true");
...
storeLocator.Store.prototype.getInfoPanelItem=function(){var a=storeLocator.Store.infoPanelCache_,b=this.getId();if(!a[b]){var c=this.getInfoPanelContent();a[b]=$('\x3cli class\x3d"store" tabindex\x3d"0"  role\x3d"button" id\x3d"store-'+this.getId()+'"\x3e'+c+"\x3c/li\x3e")[0]}return a[b]};
